...
export default class InventoryTable extends Component {
        ...
        this.props.tools.swal.fire({
            ...
            if (res['value'] === true) {
                this.props.req.delete(new_url).then(res => {
                    this.props.tools.swal.fire('Success!', 'Item Deleted!', 'success').then(() => {
                        window.location.reload(false);
                    })
                }).catch(() => {
                ...
                this.props.tools.swal.fire('Cancelled', 'The items was not removed!', 'info')
            }
        })
    }
    ...
    render() {
        ...
        for (var item in row_data) {
            ...
            row_data[item]['add'] = <MDBBtn onClick={() => this.handleAddModal(data)} className="btn btn-primary btn-block" data-toggle="modal" data-target='#AddToInvModal'>ADD</MDBBtn>
        ...
        return (
            <div>
                    ...
                    <div className="modal-dialog modal-dialog-centered modal-lg" role="document">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title" id="exampleModalLabel">EDIT PRODUCT WITH CODE : {this.state.id}</h5>
                                <button className="close" type="button" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">×</span>
                                </button>
                            </div>
                            <form id="Inventory-Edit" onReset={this.handleFormReset} onSubmit={this.handleFormSubmit}>
                ...
                <AddInventory tools={this.props.tools} add_inv={this.state.inventory} req={this.props.req}></AddInventory>
            </div>
        );
    }
}